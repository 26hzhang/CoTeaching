...
from data.mnist_th import MNIST
from data.cifar_th import CIFAR10, CIFAR100
from model_th import CNN, loss_coteaching
...
parser.add_argument('--num_workers', type=int, default=4, help='how many sub-processes to use for data loading')
...
save_dir = args.result_dir + '/' + args.dataset + '/coteaching-th/'
...
def accuracy(logit, target, topk=(1,)):
    ...
    for k in topk:
        ...
        res.append(correct_k.mul_(100.0 / input_size))
    return res
...
def train(train_loader, epoch, model1, optimizer1, model2, optimizer2):
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        images = Variable(images).cuda(device=args.gpu_idx)
        labels = Variable(labels).cuda(device=args.gpu_idx)
        ...
        if (i + 1) % args.print_freq == 0:
            print('Epoch [%d/%d], Iter [%d/%d] Training Accuracy1: %.4f%%, Training Accuracy2: %.4f%%, Loss1: %.4f, '
                  ...
                  (epoch + 1, args.n_epoch, i + 1, len(train_dataset) // batch_size, prec1, prec2,
                   loss_1.item(), loss_2.item(), sum(pure_ratio_1_list) / len(pure_ratio_1_list),
...
def main():
    ...
    print('Epoch [%d/%d] Test Accuracy on the %s test images: Model1 %.4f%% Model2 %.4f%% Pure Ratio1 %.4f%% '
          'Pure Ratio2 %.4f%%' % (epoch + 1, args.n_epoch, len(test_dataset), test_acc1, test_acc2, mean_pure_ratio1,
                                  mean_pure_ratio2))
    # save results
    with open(txtfile, "a") as myfile:
        myfile.write(
